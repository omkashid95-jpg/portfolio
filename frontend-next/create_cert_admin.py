...
export default function CertificationsAdminPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:8001/api/upload?file_type=certification', {
        method: 'POST',
        headers: {
      ...
      setFormData(prev => ({ ...prev, image_url: `http://127.0.0.1:8001${data.file_url}` }));
    } catch (err) {
      console.error(err);
...
os.makedirs('src/app/admin/certifications', exist_ok=True)
with open('src/app/admin/certifications/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
